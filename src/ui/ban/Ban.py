...
class Ban(Modal, title="Ban"):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        await interaction.response.send_message(f"Banned {member.name} for {self.reason_field.value}", ephemeral=True)
        await member.send(f"You have been banned in {guild.name} for {self.reason_field.value}")
        await Utils.log(guild, f"**Banned {member.name}#{member.discriminator}**", self.reason_field.value, interaction.user, member, interaction.created_at)
        await member.ban(reason=self.reason_field.value)